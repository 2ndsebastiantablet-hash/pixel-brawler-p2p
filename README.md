...
- Thirty-four enabled polished weapons/items for this slice: pistol, whip, teleporting ball, lightning rod, sledgehammer, slingshot, laser blaster, revolver, minigun, sniper, knife, machete, axe, wings, grabber, trident, super bomb, virgin blood, death aura, rocket, hands, super legs, holy bazooka, grappling hook, chainsaw, spikes, van, Spirit of a Fighter, cross, The Moon, Jupiter, Uranus, Mars, and Neptune.
- Optional hybrid Three.js/WebGL render layer in `src/game/render3d/` for real low-poly 3D model support beside the current 2D canvas. It now renders visual-only Jupiter sharks, Trident flood sharks through the same shark path, the centered Uranus/Saturn ring planet, the remade left-side Ring Chomper, Mars event planet models, and split Neptune boss/sea-creature models when available: Neptune's giant body sits in the background layer behind gameplay while sea creatures stay in the foreground gameplay layer. The Moon stays in the 2D background during live gameplay so it cannot cover fighters, and the layer fails closed so the 2D game still loads if WebGL is unavailable.
...
By default the layer initializes without demo actors. During active combat events it mirrors existing snapshot state with 3D Jupiter sharks, Trident flood sharks, a translucent centered Saturn-style Uranus planet with rings, a remade yellow left-side Ring Chomper, a red/orange Mars model, foreground Neptune sea creatures, and a separate background Neptune boss layer that stays behind the 2D canvas players, projectiles, platform, and HUD. The Moon event is drawn behind gameplay in the 2D canvas layer to keep the playfield readable. Existing 2D canvas event art remains the fallback when render3d is disabled or unavailable. Enable the small rotating cube demo with `?render3dDemo=1` or `localStorage.setItem("pixel-brawler-p2p.render3d.demo", "true")`. Disable WebGL with `?render3d=0` or `localStorage.setItem("pixel-brawler-p2p.render3d.disabled", "true")`. Details, current Jupiter/Uranus/Moon/Mars/Neptune model notes, and rollback instructions live in `docs/3d-rendering.md`.
...
- Right mouse: Use the held item alternate/secondary action, such as Knife throw, Axe throw/recall, Trident throw, Rocket light, Holy Bazooka ammo call, Grappling Hook pull while attached, Chainsaw stop/vent, Virgin Blood consume, or Cross Judgment Day. Pistol right click does not throw the pistol.
- With Trident equipped, press both mouse buttons together to trigger the flood super when it is off cooldown. Separate left/right clicks still use the normal stab and physical throw actions, and thrown tridents must be picked back up manually.
- With Super Bomb strapped and both hands empty, left mouse detonates a mouse-point pop blast, right mouse throws a body-limb bomb, and both mouse buttons together trigger the full-body super explosion when its long cooldown is ready. Holding a hand weapon disables these Super Bomb mouse actions until the hands are empty again.
...
- Grabber and Super Bomb are regular front/back strap utility items, not `Q`/`E` Space events. Grabber passively adds a mechanical glove arm, a third F-cycle storage slot, animated empty auto-punches, and farther pickup reach. Super Bomb arms the empty-hand mouse actions above.
...
- `F`: Safely swap the attachment string item with the current hand item, move a held compatible item onto an empty attachment, move an attachment item into an empty hand, or pick up nearby compatible gear first. With Grabber strapped, `F` rotates compatible items through hand, attachment string, and Grabber glove storage without duplicating or deleting gear.
...
- Grabber: Light regular front/back strap utility item after Wings. It is not a Space event and has no normal left-click/right-click attack. Equipping it adds a visible segmented mechanical arm with a white glove, unlocks a Grabber storage slot that accepts the same physical handheld items as the attachment string, and makes `F` cycle hand, attachment, and Grabber-held items safely. If the glove is empty, it automatically runs a real windup, extend, hit, retract, and cooldown punch animation against nearby dummies, enemies, zombies/sea creatures, and online players with hit-timed damage and strong knockback, and it extends pickup reach for nearby dropped gear.
- Trident: Balanced regular handheld item after Grabber. It can equip in hands, the attachment string, or the Grabber glove slot, but not straps or legs. Left mouse uses a mouse-aimed stab/blunt strike with a long cyan three-prong staff visual, right mouse throws the physical Trident and removes it from the held inventory until it is manually picked back up, and both mouse buttons together trigger a 60-second flood super with a 120-second cooldown. Trident hits transform targets for 40 seconds into full puffer fish, goldfish, or octopus silhouettes: puffers glide, inflate, and release poison gas; goldfish flop with weak ground movement and shoot random-sized water droplets; octopuses grab up to eight targets with left mouse and throw them with right mouse. The flood adds swimming physics and a killable shark threat while leaving normal Trident stab/throw behavior usable.
- Super Bomb: Light front/back strap utility item after Trident. It has no `Q`/`E` event and only works while both hands are empty. Left mouse creates a large mouse-point body pop explosion with fireworks, right mouse throws a detachable limb bomb that regrows after a timer, and both mouse buttons together trigger a huge full-body explosion, scatters poison body splatters, locks the user into a visible reformation state, then reforms them with one stacking weakness penalty. Missing Super Bomb legs weaken movement and block Super Legs jumps/kicks until they regrow.
- Virgin Blood: Light holy utility item after Trident. Left or right click consumes it to fully heal, gain a holy buff, and arm one revive once it is held. `F` swaps it between the attachment string and the current hand item instead of consuming it. If killed while the revive is ready, the player revives once with restored HP, invulnerability, renewed holy buff, and 30 seconds of angel wings flight using the Wings movement model. The blessing has a long cooldown and cannot chain infinite revives.
...
- Neptune: Light one-use strap-only Space item after Mars. Equip it only on the front or back strap. Press `Q`/`E` to consume it and summon a one-minute invincible ocean boss event. Neptune's giant hands rise through the platform, crush targets underneath, grab the map, then his torso, arms, neck, head, and crown rise behind gameplay without visible legs while blue text slams in and the roar shakes the screen. During the event he uses animated, readable boss attacks: ocean vomit floods the entire visible map with buoyant swimming physics before he sucks the water back up; Water Drop Rain spits a giant droplet upward that bursts into varied falling droplets; hand slams tilt the platform harder and hold the slide longer; laser eyes fire from both visible eye origins, start slightly behind the target, chase forward, and instantly kill touched targets; and sea-creature summons pop killable urchins, octopuses, giant sharks, and clown fish from seeded random valid platform positions. Urchins stick/poison targets for 10 seconds, octopus arms fling up to eight targets, giant sharks use the clean Jupiter-style low-poly shark model without blue overlay or floor clipping, clown fish flop then shoot variable water pellets, and all Neptune water/tilt/creature state cleans up when he descends.
...
- Physical Weapon Throws: Non-knife, non-pistol, non-recall physical throws create a world pickup and remove that weapon from the player's weapon inventory until the pickup is collected again. Trident uses this path for right-click throws, so a thrown Trident does not leave a duplicate in the user's hand.
...
- Grabber Arm: Grabber is a passive strap utility. Its extra glove slot is compacted in loadout sync, remote players draw the mechanical arm/held item, empty gloves auto-punch nearby valid targets through a visible windup/extend/hit/retract state machine and normal hit packets, and empty gloves extend the existing pickup/interact reach instead of adding a new button.
...
- Trident Transform And Flood: Trident stab and throw hits transform targets for 40 seconds through synced puffer, goldfish, or octopus statuses rendered as full creature silhouettes for players, dummies, and enemies. Both mouse buttons trigger a 60-second flood super with swimming physics, water visuals, a killable shark, impact sounds, and a 120-second cooldown; the flood, thrown Trident, world pickup, shark state, and form abilities live in the combat snapshot/event flow so online players see the same state without Worker/signaling changes.
- Super Bomb Body Explosions: Super Bomb is a front/back strap utility that stays dormant until both hands are empty. Empty-hand left mouse uses a true-radius mouse-point pop blast, empty-hand right mouse throws a fuse-based limb bomb that removes and later regrows a body part, and both mouse buttons trigger a full-body super with huge falloff damage/knockback, poison splatters, a visible reformation lockout, and stacking weakness. Limb loss, splatter poison, reformation, sounds, effects, and remote visuals use the existing combat snapshot/event/status flow without Worker/signaling changes.
- Jupiter Event: Jupiter is a strap-only one-use Space event. Activating it with `Q`/`E` consumes the equipped slot, broadcasts orange floaty gas, delayed footstep pressure markers, tornado state, and killable homing sharks through combat snapshots/events, mirrors those sharks as real low-poly Three.js models when render3d is available, launches step victims much farther upward than before, and cleans up only its own markers/sharks when the timer ends while allowing Moon, Uranus, Mars, Neptune, and Judgment Day to keep running.
...
- Neptune Event: Neptune is a strap-only one-use Space event. Activating it with `Q`/`E` consumes the equipped slot, broadcasts a one-minute invincible boss state with intro hands, roar text, full-screen water/flood state, stronger longer platform tilt state, two-origin behind-target laser-eye state, Water Drop Rain pellets, killable sea-creature combat bodies with seeded random pop-up spawn positions, sounds, and cleanup through combat snapshots/events. The boss model is visual-only and rendered on the background Three.js layer behind the 2D gameplay canvas; the crush zones, swimming buoyancy/drag, tilt force, chasing lasers, droplet rain, creature damage, and creature death cleanup stay in `CombatSystem` so dummies, online players, vans, enemies, and other event targets share the same authoritative hit path. The giant shark uses the clean low-poly shark path rather than a huge blue overlay body. If Moon, Jupiter, Uranus, Mars, Neptune, and Judgment overlap, the stable render priority is Uranus space/ring background, Moon background/top-floor overlay, Mars planet/beams/clones, Neptune background boss/flood/foreground creatures, Jupiter gas/tornado/markers, then players/combat; each event owns and restores only its own state.
...
Open the printed local URL, usually `http://localhost:5173`. The first screen is a controls/loading screen with keyboard keycaps and mouse controls. Continue to the main menu, press **Play**, choose a name/color, edit the loadout side panel if desired, then use **Offline Test** for local movement and combat. The character creator uses true drag/drop: drag item cards onto the front hand X, front leg X, front chest strap X, straight glowing attachment-string X, or the single back strap X. The item grid has search and category filters. The attachment string accepts any physical handheld item, including Knife, Pistol, Revolver, Virgin Blood, Rocket, Machete, Slingshot, Axe, Trident, Grappling Hook, Holy Bazooka, Chainsaw, Cross, and other held weapons; when Grabber is equipped on a front/back strap, an extra Grabber slot appears and accepts that same physical handheld set, including Trident. Body/projectable/event powers such as Hands, Death Aura, Wings, Grabber, Spikes, Van, Spirit of a Fighter, The Moon, Jupiter, Uranus, Mars, and Neptune belong on front/back straps; Super Legs belongs only on the leg X. Fresh loadouts start empty unless saved gear exists. Use **Use Default Loadout** to fill the starter preset with Pistol in the left hand, Knife in the right hand, Wings on the front strap, Death Aura on the back strap, and Virgin Blood on the attachment string. Right-click an equipped X to clear it. Pressing `F` swaps attachment and hand items safely; with Grabber strapped, `F` cycles the current hand, attachment, and Grabber glove item positions while preserving every item. Empty Grabber gloves also auto-punch nearby targets and extend pickup reach. Offline Test spawns a training dummy and shows the equipped slot HUD.
...
- Combat is a playable thirty-four-weapon/item vertical slice, not final balance.
