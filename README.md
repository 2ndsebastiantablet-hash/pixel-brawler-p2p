...
- Grabber is a regular front/back strap utility, not a `Q`/`E` Space event. When strapped, it passively adds a mechanical glove arm, a third F-cycle storage slot, empty auto-punches, and farther pickup reach.
...
- Grabber Arm: Grabber is a passive strap utility. Its extra glove slot is compacted in loadout sync, remote players draw the mechanical arm/held item, empty gloves auto-punch nearby valid targets through normal hit packets, and empty gloves extend the existing pickup/interact reach instead of adding a new button.
...
- Combat is a playable thirty-two-weapon/item vertical slice, not final balance.
