...
- Twenty-three enabled polished weapons/items for this slice: pistol, whip, teleporting ball, lightning rod, sledgehammer, slingshot, laser blaster, revolver, minigun, sniper, knife, machete, axe, wings, virgin blood, death aura, rocket, hands, super legs, holy bazooka, grappling hook, chainsaw, and spikes.
...
- `Q`: Activate the front strap slot only. If Spikes is strapped, this starts 30 seconds of Spike Mode.
- `E`: Activate the back strap slot only. If Spikes is strapped, this starts 30 seconds of Spike Mode.
...
- Spikes: Light strap-only weapon after Chainsaw. Equip it only on the front or back strap; it cannot go in hands, attachment, or legs. Press `Q`/`E` to activate 30 seconds of Spike Mode, which hides/blocks the hand item without deleting it. During Spike Mode, left or right mouse clicks place vertical poison spikes at the mouse world position with no per-spike cooldown, capped active spikes, fast growth, white-to-dark tips, green poison cores, and crumble particles when removed. Spikes impale dummies, enemies, and online players, hold them in place, apply stronger 18-second spike poison with slow DOT, then release on disintegration, mode end, death, or failsafe. When the mode ends, all owner spikes disintegrate and a 60-second cooldown starts.
...
- Spike Mode: Spikes are a strap mode rather than normal left/right attacks. Spike mode, spike poison, hand blocking, spike spawn visuals, impale hit packets, and disintegration particles use the existing combat snapshot/status/event flow so remote players can see spikes and receive impale/poison effects without changing signaling.
