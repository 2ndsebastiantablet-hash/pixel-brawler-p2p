...
- Twenty-eight enabled polished weapons/items for this slice: pistol, whip, teleporting ball, lightning rod, sledgehammer, slingshot, laser blaster, revolver, minigun, sniper, knife, machete, axe, wings, virgin blood, death aura, rocket, hands, super legs, holy bazooka, grappling hook, chainsaw, spikes, van, Spirit of a Fighter, cross, The Moon, and Jupiter.
...
- Jupiter: Light one-use strap-only Space item after The Moon. Equip it only on the front or back strap. Press `Q`/`E` to consume it and start a one-minute Jupiter event that can stack with other global events: an earthquake cracks the floor with deadly holes, orange gas makes gravity floaty, and a dark green shark tornado sucks in players/vans while spawning killable flying homing sharks. The tornado core and deadly holes kill through the normal combat/respawn flow; sharks use combatant health so weapons can destroy them.
...
- Jupiter Event: Jupiter is a strap-only one-use Space event. Activating it with `Q`/`E` consumes the equipped slot, broadcasts quake holes, orange floaty gas, tornado state, and killable homing sharks through combat snapshots/events, and cleans up its own holes/sharks when the timer ends while allowing Moon and Judgment Day to keep running.
