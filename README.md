...
- `Q`: Activate the front strap slot only. If Spikes is strapped, this starts 30 seconds of Spike Mode; if Van is strapped, this spawns or absorbs the physics van; if Spirit of a Fighter is strapped, this starts Beat Focus.
- `E`: Activate the back strap slot only. If Spikes is strapped, this starts 30 seconds of Spike Mode; if Van is strapped, this spawns or absorbs the physics van; if Spirit of a Fighter is strapped, this starts Beat Focus.
...
- Spirit of a Fighter: Light strap-only rhythm focus mode after Van. Equip it only on the front or back strap, then press `Q`/`E` to activate up to 25 seconds of Beat Focus with a 60-second cooldown. While active, hand weapons are hidden/disabled without being deleted; left click punches, right click grabs/throws, and `Shift` flash-steps only when timed on the beat. Perfect/good timing builds combos, flurries, counters, and precision finishers. A missed beat, off-beat input, whiff, or incoming hit immediately ends the mode and applies about 9 seconds of Winded movement penalty.
...
- Spirit Focus: Spirit of a Fighter is a strap mode rather than a normal hand attack. Beat Focus and Winded are synced statuses, the local timing HUD shows beat/combo/feedback/cooldown, and the focus effects stay visual/local enough to avoid slowing remote multiplayer simulation.
...
- Combat is a playable twenty-five-weapon/item vertical slice, not final balance.
