...
- Death Aura: Dark utility item after Virgin Blood. Left or right click activates a dark aura around the user that freezes and damages nearby targets. Missing health powers it up: low HP makes the radius larger, damage ticks stronger, freeze longer, and smoke darker.
- Rocket: Deployable ridable explosive after Death Aura. Left click places one rocket, right click lights it when nearby or standing on it, and standing on it attaches the rider. Lit rockets launch straight briefly, then veer into chaotic fire-trail flight; `Space` jumps off before the explosion.
...
- Death Aura Field: Death Aura uses per-target tick cooldowns to avoid permanent stun-locking while still freezing and draining targets inside the current radius.
- Rocket Projectiles: Rockets are tracked as owner projectiles, support one active rocket per owner, attach riders locally, and use burst damage/knockback events for online targets on explosion.
